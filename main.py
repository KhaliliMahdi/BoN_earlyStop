...
filename = f"records_GPU={rank}.json"
...
try:
    with open('indices.csv', "r") as f:
        reader = csv.reader(f)
        completed = [int(i) for row in reader for i in row if i]
except FileNotFoundError:
    completed = []
    
print(f"Completed indices: {completed}")


for index, prompt in zip(indices,prompts):
    if index in completed:
        print(f"Skipping index {index} as it is already completed.")
        continue
    ...
    result = tokenizer.batch_decode(output_sequences, skip_special_tokens=True)
        ...
        #score Generation
    score = []
    for response in tqdm(result,ncols=80, dynamic_ncols=True):
        l = len(prompt)
    ...
    for batch in tqdm(p,ncols=80, dynamic_ncols=True):
